...
- **Accessibility is mandatory (WCAG 2.1 AA)** — for everyone, including specially-abled users; it's a patient-safety requirement. Keyboard-operable everything (no `div`/`span` `onClick` — use `<button>` or `role`+`tabIndex`+`onKeyDown`), visible 2px focus ring, `aria-label` on every icon-only control, real labels + wired errors on inputs, never colour-alone for meaning, honour `prefers-reduced-motion`. Enforced by **strict Biome a11y rules** (all `error`) + the `@/components/ui` seam. Full rules + the ARIA index: `medbrains/docs/ACCESSIBILITY.md`.

...
/**
 * A row a patient can actually operate.
 *
 * This was a plain `View` with `onTouchEnd`: invisible to VoiceOver, no press
 * feedback, fires even if the finger slides off, and an action with no handler
...
function ActionRow({ action }: { action: ModuleAction }) {
  ...
  const description = isUnavailable
    ? [action.description, "Not available in the app yet."].filter(Boolean).join(" ")
    : action.description;

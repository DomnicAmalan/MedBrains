  ...
  const grouped = useMemo(() => {
    ...
    for (const item of items) {
      ...
      if (parsed && !("text" in parsed)) {
        slots = parsed.time_slots?.length
          ? parsed.time_slots
      ...
      {grouped.prnDrugs.length > 0 && (
        <Card padding="sm" radius="md" withBorder className={classes.timeGroupCard}>
          <Group gap="xs" mb="xs">
            ...
            <div>
              <Text fw={600} size="sm">
                As Needed (PRN / SOS / STAT)
              </Text>
              <Text size="xs" c="dimmed">
                Take only when required
              </Text>
            </div>
            <Badge size="sm" variant="light" color="warning" ml="auto">
              {grouped.prnDrugs.length}
            </Badge>
          </Group>
          <Stack gap={4}>
...
function DrugRow({ drug }: { drug: GroupedDrug }) {
  ...
  return (
    <Paper
      ...
      style={{ borderLeft: `3px solid var(--mantine-color-${routeColor}-5)` }}
    >
      <Group justify="space-between" wrap="nowrap">
        <div style={{ flex: 1 }}>
          ...
          <Group gap={8}>
            ...
            {drug.route && (
              <Badge size="xs" variant="light" color={routeColor}>
                {drug.route}
              </Badge>
            )}

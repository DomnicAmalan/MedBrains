...
interface GeneratedDeviceConfig {
  protocol_config: Record<string, unknown>;
  field_mappings: unknown[];
  data_transforms: unknown[];
  qc_config: Record<string, unknown>;
      ...
      {manufacturers && (
        <Group gap="xs">
          <Text size="xs" c="dimmed" fw={500}>
            {manufacturers.reduce((sum: number, m: ManufacturerSummary) => sum + m.model_count, 0)}{" "}
            adapters from {manufacturers.length} manufacturers
          </Text>
        </Group>
      )}
          ...
          {adapters?.map((a: DeviceAdapterCatalog) => (
            <Card key={a.id} withBorder p="md">
              ...
              <Group gap="xs" mb="sm">
                ...
                >
                  {((a as unknown as Record<string, string>).data_direction ?? "producer").replace(
                    /_/g,
                    " ",
                  )}
                </Badge>
              ...
              {a.default_config &&
                Array.isArray((a.default_config as Record<string, unknown>).features) && (
                  <Group gap={4} mb="xs">
                    ...
                    {((a.default_config as Record<string, unknown>).features as string[]).length >
                      4 && (
                      <Text
                        size="xs"
                        c="dimmed"
                      >{`+${((a.default_config as Record<string, unknown>).features as string[]).length - 4} more`}</Text>
                    )}
                  </Group>
                )}
  ...
  return (
    <Stack gap="md">
      ...
      <Card withBorder p="md">
        ...
        <Stack gap={4}>
          <Text size="xs" c="dimmed">
            1. Device sends data &rarr; bridge parses it &rarr; POSTs to /api/device-ingest/module
          </Text>
          <Text size="xs" c="dimmed">
            2. Server finds matching routing rules (by device or adapter type)
          </Text>
          <Text size="xs" c="dimmed">
            3. Extracts match value from parsed payload (e.g., OBR.3 = sample barcode)
          </Text>
          <Text size="xs" c="dimmed">
            4. Looks up existing order/record &rarr; creates/updates entity
          </Text>
          <Text size="xs" c="dimmed">
            5. If critical value detected and alerts on &rarr; sends notification
          </Text>
        </Stack>
      </Card>
    ...
    <Modal opened={opened} onClose={onClose} title={<Text fw={700}>Add Routing Rule</Text>}>
      <Stack>
        ...
        <TextInput
          label="Adapter code"
          value={adapterCode}
          onChange={(e) => setAdapterCode(e.currentTarget.value)}
        ...
        <Group grow>
          <Select
            ...
            value={targetModule}
            onChange={(v) => setTargetModule(v ?? "lab")}
          />
          <Select
            label="Match strategy"
            data={[
              { value: "order_id", label: "Order ID" },
              { value: "sample_barcode", label: "Sample barcode" },
              { value: "patient_id", label: "Patient ID" },
              { value: "accession_number", label: "Accession #" },
              { value: "uhid", label: "UHID" },
              { value: "custom", label: "Custom" },
            ]}
            value={matchStrategy}
            onChange={(v) => setMatchStrategy(v ?? "order_id")}
          />
        </Group>
        <TextInput
          label="Match field path"
          value={matchField}
          onChange={(e) => setMatchField(e.currentTarget.value)}
        ...
        <TextInput
          label="Target entity"
          value={targetEntity}
          onChange={(e) => setTargetEntity(e.currentTarget.value)}
      ...
      {step === 2 && aiConfig && selectedAdapter && (
        <Stack>
          <Group justify="space-between">
            <div>
              <Text size="sm" fw={600}>
                {selectedAdapter.manufacturer} {selectedAdapter.model}
              </Text>
              <Text size="xs" c="dimmed">
        ...
        <Stack>
          ...
          <Group grow>
            ...
            <TextInput
              ...
              placeholder={String(aiConfig?.default_port ?? "")}
              w={100}
            />
          </Group>
          <TextInput

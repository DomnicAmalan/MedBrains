...
function OtSection({ params }: { params: { from?: string; to?: string } }) {
  ...
  const columns: Column<OtUtilizationRow>[] = [
    { key: "room", label: "Room", render: (r) => <Text size="sm">{r.room_name}</Text> },
    {
      key: "bookings",
      label: "Bookings",
      render: (r) => <Text size="sm">{r.total_bookings}</Text>,
    },
    { key: "completed", label: "Completed", render: (r) => <Text size="sm">{r.completed}</Text> },
    {
      ...
      label: "Cancelled",
      render: (r) => (
        <Text size="sm" c={r.cancelled > 0 ? "danger" : undefined}>
          {r.cancelled}
        </Text>
      ),
    },
    {
      key: "avg_dur",
      label: "Avg Duration",
      render: (r) => <Text size="sm">{r.avg_duration_mins}m</Text>,
    },
    {
      key: "util",
      label: "Utilization",
      render: (r) => (
        <Text
          size="sm"
          fw={600}
          c={r.utilization_pct > 85 ? "success" : r.utilization_pct < 50 ? "danger" : undefined}
        >
...
function ErSection({ params }: { params: { from?: string; to?: string } }) {
  ...
  const columns: Column<ErVolumeRow>[] = [
    ...
    { key: "non", label: "Non Urgent", render: (r) => <Text size="sm">{r.non_urgent}</Text> },
    {
      key: "dtd",
      label: "Avg D2D (min)",
      render: (r) => <Text size="sm">{r.avg_door_to_doctor_mins.toFixed(0)}</Text>,
    },
  ];
...
function IndicatorsSection({ params }: { params: { from?: string; to?: string } }) {
  ...
  const columns: Column<ClinicalIndicatorRow>[] = [
    { key: "period", label: "Period", render: (r) => <Text size="sm">{r.period}</Text> },
    {
      ...
      label: "Mortality %",
      render: (r) => (
        <Text size="sm" c={r.mortality_rate > 2 ? "danger" : undefined}>
          {pct(r.mortality_rate)}
        </Text>
      ),
    },
    {
      ...
      label: "Infection %",
      render: (r) => (
        <Text size="sm" c={r.infection_rate > 5 ? "danger" : undefined}>
          {pct(r.infection_rate)}
        </Text>
      ),
    },
    {
      key: "readm",
      label: "Readmission %",
      render: (r) => <Text size="sm">{pct(r.readmission_rate)}</Text>,
    },
    {
      key: "alos",
      label: "ALOS (days)",
      render: (r) => <Text size="sm">{r.avg_los_days.toFixed(1)}</Text>,
    },
  ];
      ...
      {latest && (
        <SimpleGrid cols={4}>
          <StatCard
            ...
            value={pct(latest.mortality_rate)}
            color={latest.mortality_rate > 2 ? "danger" : undefined}
          />
          <StatCard
            label="Infection Rate"
            value={pct(latest.infection_rate)}
            color={latest.infection_rate > 5 ? "danger" : undefined}
          />

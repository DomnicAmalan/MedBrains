...
function InfusionTracker({
  ...
  const saveMut = useMutation({
    mutationFn: async (newInfusion: InfusionEntry) => {
      ...
      return api.createIcuFlowsheet(admissionId, {
        infusions: updated as Record<string, unknown>[],
      });
    },
    onSuccess: () => {
...
function FlowsheetsTab({ admissionId }: { admissionId: string }) {
  ...
  const createMut = useMutation({
    mutationFn: (data: CreateIcuFlowsheetRequest) => api.createIcuFlowsheet(admissionId, data),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["icu-flowsheets", admissionId] });
        ...
        <Stack>
          <Group grow>
            <NumberInput
              ...
              onChange={(v) => setForm({ ...form, heart_rate: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="SpO2 %"
              value={form.spo2 ?? ""}
              onChange={(v) => setForm({ ...form, spo2: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              onChange={(v) => setForm({ ...form, systolic_bp: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Diastolic BP"
              value={form.diastolic_bp ?? ""}
              onChange={(v) => setForm({ ...form, diastolic_bp: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              onChange={(v) =>
                setForm({ ...form, respiratory_rate: v === "" ? undefined : Number(v) })
              }
            />
            <NumberInput
              label="Temp °C"
              ...
              onChange={(v) => setForm({ ...form, temperature: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              decimalScale={1}
              value={form.cvp ?? ""}
              onChange={(v) => setForm({ ...form, cvp: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="MAP"
              value={form.mean_arterial_bp ?? ""}
              onChange={(v) =>
                setForm({ ...form, mean_arterial_bp: v === "" ? undefined : Number(v) })
              }
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              onChange={(v) => setForm({ ...form, intake_ml: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Output mL"
              value={form.output_ml ?? ""}
              onChange={(v) => setForm({ ...form, output_ml: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              label="Urine mL"
              value={form.urine_ml ?? ""}
              onChange={(v) => setForm({ ...form, urine_ml: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Drain mL"
              value={form.drain_ml ?? ""}
              onChange={(v) => setForm({ ...form, drain_ml: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Textarea
            label="Notes"
        ...
        <Stack>
          ...
          <Group grow>
            <NumberInput
              ...
              decimalScale={1}
              value={form.fio2 ?? ""}
              onChange={(v) => setForm({ ...form, fio2: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="PEEP"
              decimalScale={1}
              value={form.peep ?? ""}
              onChange={(v) => setForm({ ...form, peep: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              label="Tidal Volume mL"
              value={form.tidal_volume ?? ""}
              onChange={(v) => setForm({ ...form, tidal_volume: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Resp Rate"
              value={form.respiratory_rate ?? ""}
          ...
          <Group grow>
            <NumberInput
              ...
              decimalScale={1}
              value={form.pip ?? ""}
              onChange={(v) => setForm({ ...form, pip: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Plateau"
              decimalScale={1}
              value={form.plateau_pressure ?? ""}
              onChange={(v) =>
                setForm({ ...form, plateau_pressure: v === "" ? undefined : Number(v) })
              }
            />
          </Group>
          ...
          <Group grow>
            <NumberInput
              ...
              decimalScale={2}
              value={form.ph ?? ""}
              onChange={(v) => setForm({ ...form, ph: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="PaO2"
              decimalScale={1}
              value={form.pao2 ?? ""}
              onChange={(v) => setForm({ ...form, pao2: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              decimalScale={1}
              value={form.paco2 ?? ""}
              onChange={(v) => setForm({ ...form, paco2: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="HCO3"
              decimalScale={1}
              value={form.hco3 ?? ""}
              onChange={(v) => setForm({ ...form, hco3: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              decimalScale={1}
              value={form.sao2 ?? ""}
              onChange={(v) => setForm({ ...form, sao2: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Lactate"
              decimalScale={1}
              value={form.lactate ?? ""}
              onChange={(v) => setForm({ ...form, lactate: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Textarea
            label="Notes"
...
function ScoresTab({ admissionId }: { admissionId: string }) {
  ...
  const createMut = useMutation({
    mutationFn: (data: CreateIcuScoreRequest) => api.createIcuScore(admissionId, data),
    onSuccess: () => {
  ...
  const columns = [
    ...
    {
      key: "predicted_mortality" as const,
      label: "Mortality %",
      ...
      <Drawer opened={opened} onClose={close} title="Record ICU Score" position="right" size="sm">
        <Stack>
          ...
          <NumberInput
            ...
            onChange={(v) => setForm({ ...form, score_value: v === "" ? 0 : Number(v) })}
          />
          <NumberInput
            label="Predicted Mortality %"
            decimalScale={1}
            value={form.predicted_mortality ?? ""}
            onChange={(v) =>
              setForm({ ...form, predicted_mortality: v === "" ? undefined : Number(v) })
            }
          />
          <Textarea
            label="Notes"
...
function DevicesTab({ admissionId }: { admissionId: string }) {
  ...
  const createMut = useMutation({
    mutationFn: (data: CreateIcuDeviceRequest) => api.createIcuDevice(admissionId, data),
    onSuccess: () => {
  ...
  const removeMut = useMutation({
    mutationFn: (deviceId: string) => api.removeIcuDevice(admissionId, deviceId),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["icu-devices", admissionId] });
      ...
      <Drawer opened={opened} onClose={close} title="Track New Device" position="right" size="sm">
        <Stack>
          <Select
            ...
            onChange={(v) =>
              setForm({ ...form, device_type: (v ?? "central_line") as IcuDeviceType })
            }
          />
          <TextInput
            label="Insertion Site"
            value={form.site ?? ""}
            onChange={(e) => setForm({ ...form, site: e.currentTarget.value || undefined })}
          />
          <Textarea
            label="Notes"
            value={form.notes ?? ""}
        ...
        <Stack>
          ...
          {canManage && (
            <>
              ...
              <Checkbox
                label="Compliant"
                checked={bundleForm.is_compliant}
                onChange={(e) =>
                  setBundleForm({ ...bundleForm, is_compliant: e.currentTarget.checked })
                }
              />
              <Checkbox
                label="Device Still Needed"
                checked={bundleForm.still_needed}
                onChange={(e) =>
                  setBundleForm({ ...bundleForm, still_needed: e.currentTarget.checked })
                }
              />
              <Textarea
                label="Notes"
                ...
                onChange={(e) =>
                  setBundleForm({ ...bundleForm, notes: e.currentTarget.value || undefined })
                }
              />
              <Button loading={bundleMut.isPending} onClick={() => bundleMut.mutate(bundleForm)}>
                Save Check
              </Button>
            </>
          )}
...
function NutritionTab({ admissionId }: { admissionId: string }) {
  ...
  const createMut = useMutation({
    mutationFn: (data: CreateIcuNutritionRequest) => api.createIcuNutrition(admissionId, data),
    onSuccess: () => {
      ...
      <Drawer opened={opened} onClose={close} title="Record Nutrition" position="right" size="sm">
        <Stack>
          ...
          <TextInput
            ...
            onChange={(e) => setForm({ ...form, formula_name: e.currentTarget.value || undefined })}
          />
          <Group grow>
            <NumberInput
              label="Rate mL/hr"
              decimalScale={1}
              value={form.rate_ml_hr ?? ""}
              onChange={(v) => setForm({ ...form, rate_ml_hr: v === "" ? undefined : Number(v) })}
            />
            <NumberInput
              label="Volume mL"
              value={form.volume_ml ?? ""}
              onChange={(v) => setForm({ ...form, volume_ml: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Group grow>
            <NumberInput
              ...
              onChange={(v) =>
                setForm({ ...form, calories_kcal: v === "" ? undefined : Number(v) })
              }
            />
            <NumberInput
              label="Protein g"
              decimalScale={1}
              value={form.protein_gm ?? ""}
              onChange={(v) => setForm({ ...form, protein_gm: v === "" ? undefined : Number(v) })}
            />
          </Group>
          <Textarea
            label="Notes"
...
function NeonatalTab({ admissionId }: { admissionId: string }) {
  ...
  const createMut = useMutation({
    mutationFn: (data: CreateIcuNeonatalRequest) => api.createIcuNeonatalRecord(admissionId, data),
    onSuccess: () => {
        ...
        <Stack>
          <Group grow>
            <NumberInput
              ...
              onChange={(v) =>
                setForm({ ...form, gestational_age_weeks: v === "" ? undefined : Number(v) })
              }
            />
            <NumberInput
              label="Birth Weight (g)"
              value={form.birth_weight_gm ?? ""}
              onChange={(v) =>
                setForm({ ...form, birth_weight_gm: v === "" ? undefined : Number(v) })
              }
            />
          </Group>
          <NumberInput
            label="Current Weight (g)"
            value={form.current_weight_gm ?? ""}
            onChange={(v) =>
              setForm({ ...form, current_weight_gm: v === "" ? undefined : Number(v) })
            }
          />
          <Group grow>
            <NumberInput
              ...
              decimalScale={1}
              value={form.bilirubin_total ?? ""}
              onChange={(v) =>
                setForm({ ...form, bilirubin_total: v === "" ? undefined : Number(v) })
              }
            />
            <NumberInput
              label="Bilirubin Direct"
              decimalScale={1}
              value={form.bilirubin_direct ?? ""}
              onChange={(v) =>
                setForm({ ...form, bilirubin_direct: v === "" ? undefined : Number(v) })
              }
            />
          </Group>
          <Checkbox
            label="Phototherapy Active"
            checked={form.phototherapy_active ?? false}
            onChange={(e) => setForm({ ...form, phototherapy_active: e.currentTarget.checked })}
          />
          {form.phototherapy_active && (
            <NumberInput
              label="Phototherapy Hours"
              value={form.phototherapy_hours ?? ""}
              onChange={(v) =>
                setForm({ ...form, phototherapy_hours: v === "" ? undefined : Number(v) })
              }
            />
          )}
          <Group grow>
            <TextInput
              ...
              value={form.breast_milk_type ?? ""}
              onChange={(e) =>
                setForm({ ...form, breast_milk_type: e.currentTarget.value || undefined })
              }
            />
            <NumberInput
              label="Breast Milk Volume mL"
              value={form.breast_milk_volume_ml ?? ""}
              onChange={(v) =>
                setForm({ ...form, breast_milk_volume_ml: v === "" ? undefined : Number(v) })
              }
            />
          </Group>
          <Group grow>
            <TextInput
              ...
              value={form.hearing_screen_result ?? ""}
              onChange={(e) =>
                setForm({ ...form, hearing_screen_result: e.currentTarget.value || undefined })
              }
            />
            <TextInput
              label="Sepsis Screen"
              value={form.sepsis_screen_result ?? ""}
              onChange={(e) =>
                setForm({ ...form, sepsis_screen_result: e.currentTarget.value || undefined })
              }
            />
          </Group>
          <PatientSearchSelect
            label="Mother Patient"
            value={form.mother_patient_id ?? ""}
            onChange={(id) => setForm({ ...form, mother_patient_id: id || undefined })}
          />
          <Textarea
            label="Notes"
            value={form.notes ?? ""}

...
export function EndoscopyPage() {
  ...
  const procCols: Column<EndoscopyProcedure>[] = [
    ...
    },
    {
      key: "sedation",
      label: "Sedation",
      render: (r) => <Text size="sm">{r.sedation_type ?? "None"}</Text>,
    },
    {
      key: "biopsy",
      label: "Biopsy",
    ...
    },
    {
      key: "aldrete",
      label: "Aldrete (Post)",
      render: (r) => <Text size="sm">{r.aldrete_score_post ?? "—"}</Text>,
    },
    {
      key: "date",
      label: "Date",
  ...
  const scopeCols: Column<EndoscopyScope>[] = [
    ...
    { key: "model", label: "Model", render: (r) => <Text size="sm">{r.model ?? "—"}</Text> },
    {
      key: "scope_type",
      label: "Type",
      render: (r) => <Text size="sm">{r.scope_type ?? "—"}</Text>,
    },
    {
      key: "status",
      label: "Status",
    ...
    { key: "total_uses", label: "Uses", render: (r) => <Text size="sm">{r.total_uses}</Text> },
    {
      ...
      render: (r) => (
        <Text size="sm">
          {r.last_hld_at ? new Date(r.last_hld_at).toLocaleDateString() : "Never"}
        </Text>
      ),
    },
    {
      key: "culture",
      label: "Last Culture",
      render: (r) => <Text size="sm">{r.last_culture_result ?? "—"}</Text>,
    },
  ];
  ...
  const reprocCols: Column<EndoscopyReprocessing>[] = [
    {
      key: "scope_id",
      label: "Scope",
      render: (r) => <Text size="sm">{r.scope_id.slice(0, 8)}</Text>,
    },
    {
      key: "leak_test",
      label: "Leak Test",
    ...
    },
    {
      key: "chemical",
      label: "Chemical",
      render: (r) => <Text size="sm">{r.hld_chemical ?? "—"}</Text>,
    },
    {
      key: "soak",
      label: "Soak (min)",
      render: (r) => <Text size="sm">{r.hld_soak_minutes ?? "—"}</Text>,
    },
    {
      key: "result",
      label: "HLD Result",
  ...
  return (
    <div>
      ...
      >
        <Stack>
          ...
          <TextInput
            ...
            onChange={(e) => setProcForm((p) => ({ ...p, procedure_type: e.currentTarget.value }))}
          />
          <Select
            label="Scope"
            data={scopes
              .filter((s) => s.status === "available")
              .map((s) => ({ value: s.id, label: `${s.serial_number} (${s.model ?? "Unknown"})` }))}
            value={procForm.scope_id ?? null}
            onChange={(v) => setProcForm((p) => ({ ...p, scope_id: v ?? undefined }))}
          />
          <TextInput
            label="Sedation Type"
            value={procForm.sedation_type ?? ""}
            onChange={(e) => setProcForm((p) => ({ ...p, sedation_type: e.currentTarget.value }))}
          />
          <NumberInput
            label="Aldrete Score (Pre)"
            value={procForm.aldrete_score_pre ?? ""}
            onChange={(v) =>
              setProcForm((p) => ({
                ...p,
                aldrete_score_pre: typeof v === "number" ? v : undefined,
              }))
            }
          />
          <NumberInput
            label="Aldrete Score (Post)"
            value={procForm.aldrete_score_post ?? ""}
            onChange={(v) =>
              setProcForm((p) => ({
                ...p,
                aldrete_score_post: typeof v === "number" ? v : undefined,
              }))
            }
          />
      ...
      <Drawer
        opened={scopeOpen}
        onClose={scopeHandlers.close}
        title="Register New Scope"
        size="md"
        position="right"
      >
        <Stack>
          <TextInput
            label="Serial Number"
            required
            value={scopeForm.serial_number}
            onChange={(e) => setScopeForm((p) => ({ ...p, serial_number: e.currentTarget.value }))}
          />
          <TextInput
            label="Model"
            value={scopeForm.model ?? ""}
            onChange={(e) => setScopeForm((p) => ({ ...p, model: e.currentTarget.value }))}
          />
          <TextInput
            label="Scope Type"

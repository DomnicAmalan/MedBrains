...
use medbrains_server_core::notification_hub::{EventScope, NotificationEvent, user_topic};
...
pub fn publish_board_signal(
    ...
    publish_to_board(
        state,
        tenant_id,
        medbrains_server_core::notification_hub::department_topic(department_id),
        kind,
        entity_type,
        entity_id,
    );
...
pub fn publish_surface_board_signal(
    ...
    publish_to_board(
        state,
        tenant_id,
        medbrains_server_core::notification_hub::board_topic(surface),
        kind,
        entity_type,
        entity_id,
    );
...
pub async fn notifications_ws_handler(
    ...
    let token = headers
        ...
        .and_then(|c| medbrains_server_core::middleware::auth::parse_cookie_value(c, "access_token").map(str::to_owned))
        .or(q.token)
        .ok_or(AppError::Unauthorized)?;
    let claims = medbrains_server_core::middleware::auth::decode_and_validate(&token, &state.jwt_decoding_key)?;
...
fn socket_topics(claims: &Claims) -> Vec<String> {
    ...
    for department_id in claims.department_ids.iter().take(MAX_DEPARTMENT_TOPICS) {
        if seen.insert(*department_id) {
            topics.push(medbrains_server_core::notification_hub::department_topic(*department_id));
        }
    }
    for (surface, required_any) in SURFACE_BOARD_PERMISSIONS {
        let permitted = medbrains_server_core::middleware::authorization::is_bypass_role(claims)
            || required_any
                .iter()
                .any(|needed| claims.permissions.iter().any(|held| held == needed));
        if permitted {
            topics.push(medbrains_server_core::notification_hub::board_topic(surface));
        }
    }
    topics
}

...
pub async fn delete_user(
    ...
    if let Err(e) = medbrains_vpn::revoke_user_devices(&state, claims.tenant_id, id).await {
...
pub async fn update_role_field_access(
    ...
    require_permission(&claims, permissions::admin::roles::UPDATE)?;
    validate_field_access_map(&body.field_access)?;
...
/// Setup / configuration (access groups, invitations, config import/export, health, onboarding) authed routes.
pub fn router() -> axum::Router<AppState> {
    axum::Router::new()
        ...
        .route(
            ...
            put(update_access_group).delete(delete_access_group),
        )
        .route(
            "/api/access-groups/{id}/members",
            get(list_access_group_members).post(add_access_group_member),
        )
        .route(
            "/api/access-groups/{group_id}/members/{user_id}",
            delete(remove_access_group_member),
        )
        .route(
        ...
        .route(
            ...
            put(update_facility).delete(delete_facility),
        )
        .route(
            "/api/setup/locations",
        ...
        .route(
            ...
            get(list_location_directory),
        )
        .route(
            "/api/setup/locations/{id}/staff",
            get(list_location_staff).post(assign_location_staff),
        )
        .route(
            "/api/setup/locations/{id}/staff/{user_id}",
            delete(remove_location_staff),
        )
        .route(
        ...
        .route(
            ...
            put(update_user_access_matrix),
        )
        .route("/api/setup/modules", get(list_modules))
        .route("/api/setup/modules/{code}", put(update_module))
        .route(
            "/api/setup/sequences",
            get(list_sequences).post(create_sequence),
        )
        .route(
        ...
        .route(
            "/api/setup/bed-types",
            get(list_bed_types).post(create_bed_type),
        )
        .route(
            "/api/setup/bed-types/{id}",
            put(update_bed_type).delete(delete_bed_type),
        )
        .route(
            "/api/setup/tax-categories",
            get(list_tax_categories).post(create_tax_category),
        )
        .route(
            "/api/setup/tax-categories/{id}",
            put(update_tax_category).delete(delete_tax_category),
        )
        .route(
        ...
        .route(
            ...
            get(list_user_facilities).put(assign_user_facilities),
        )
        .route(
            "/api/setup/facilities/{id}/auto-compliance",
            post(auto_create_compliance),
        )
        .route(
            "/api/setup/print-templates",
            get(get_print_templates).put(upsert_print_template),
        )
        .route(
            "/api/setup/masters/religions",
            get(list_master_religions).post(create_master_religion),
        )
        .route(
            "/api/setup/masters/religions/{id}",
            put(update_master_religion).delete(delete_master_religion),
        )
        .route(
            "/api/setup/masters/occupations",
            get(list_master_occupations).post(create_master_occupation),
        )
        .route(
            "/api/setup/masters/occupations/{id}",
            put(update_master_occupation).delete(delete_master_occupation),
        )
        .route(
            "/api/setup/masters/relations",
            get(list_master_relations).post(create_master_relation),
        )
        .route(
            "/api/setup/masters/relations/{id}",
            put(update_master_relation).delete(delete_master_relation),
        )
        .route(
            "/api/setup/masters/insurance-providers",
            get(list_insurance_providers).post(create_insurance_provider),
        )
        .route(
            "/api/setup/masters/insurance-providers/{id}",
            put(update_insurance_provider).delete(delete_insurance_provider),
        )
        .route("/api/setup/users/bulk", post(bulk_create_users))
        .route(

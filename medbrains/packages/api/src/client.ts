import type {
  ...
  UpdatePharmacyOrderItemRequest,
...
export const api = {
  ...
  listSharingSubjects: () =>
    request<{
      users: Array<{ id: string; label: string; subtitle: string | null }>;
      roles: Array<{ id: string; label: string; subtitle: string | null }>;
      departments: Array<{ id: string; label: string; subtitle: string | null }>;
      groups: Array<{ id: string; label: string; subtitle: string | null }>;
    }>("/sharing/subjects"),
  ...
  createIamAccessRequest: (data: CreateIamAccessRequest) =>
    request<IamAccessRequest>("/iam/access-requests", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  approveIamAccessRequest: (id: string, data?: { note?: string }) =>
    request<IamAccessRequest>(`/iam/access-requests/${id}/approve`, {
      method: "POST",
      body: JSON.stringify(data ?? {}),
    }),
  rejectIamAccessRequest: (id: string, data?: { note?: string }) =>
    request<IamAccessRequest>(`/iam/access-requests/${id}/reject`, {
      method: "POST",
      body: JSON.stringify(data ?? {}),
    }),
  revokeIamAccessRequest: (id: string, data?: { reason?: string }) =>
    request<IamAccessRequest>(`/iam/access-requests/${id}/revoke`, {
      method: "POST",
      body: JSON.stringify(data ?? {}),
    }),
  ...
  /** Complete a camp token without sending the patient to the next station. */
  finishToken: (id: string) => request<ModuleToken>(`/tokens/${id}/finish`, { method: "POST" }),
  ...
  listBlog: (params?: { include_drafts?: boolean }) =>
    request<BlogPostListItem[]>(`/blog${params?.include_drafts ? "?include_drafts=true" : ""}`),
  ...
  createNurseVital: (data: CreateVitalRequest & { encounter_id: string }) =>
    request<Vital>("/nurse/vitals", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  importChargeMaster: (data: CsvImportRequest) =>
    request<CsvImportResult>("/billing/charge-master/import", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  updatePharmacyOrderItem: (
    orderId: string,
    itemId: string,
    data: UpdatePharmacyOrderItemRequest,
  ) =>
    request<PharmacyOrderDetailResponse>(`/pharmacy/orders/${orderId}/items/${itemId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  removePharmacyOrderItem: (orderId: string, itemId: string) =>
    request<PharmacyOrderDetailResponse>(`/pharmacy/orders/${orderId}/items/${itemId}`, {
      method: "DELETE",
    }),
  dispenseOrder: (
  ...
  getMySignatureCredentials: () => request<SignatureCredential[]>("/signatures/my-credentials"),
  previewSignature: (data: SignPreviewRequest) => {
    const qs = new URLSearchParams({
      record_type: data.record_type,
      record_id: data.record_id,
    });
    if (data.signer_role) qs.set("signer_role", data.signer_role);
    if (data.legal_class) qs.set("legal_class", data.legal_class);
    return request<SignPreviewResponse>(`/signatures/preview?${qs.toString()}`);
  },

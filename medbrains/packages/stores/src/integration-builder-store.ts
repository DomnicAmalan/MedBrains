...
export interface IntegrationBuilderActions {
  ...
  addNode: (template: IntegrationNodeTemplate, position: { x: number; y: number }) => string;
...
export type IntegrationBuilderStore = IntegrationBuilderState & IntegrationBuilderActions;
...
export const useIntegrationBuilderStore = create<IntegrationBuilderStore>((set, get) => ({
  // State
  pipeline: { ...EMPTY_META },
  nodes: [],
  edges: [],

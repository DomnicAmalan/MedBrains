...
import type { PatientFlowReadinessItem } from "./patient-flow.js";
...
const BLOCKED_REASON_FALLBACK_LABELS = {
...
} satisfies Record<ClinicalJourneyBlockedReason, string>;

const JOURNEY_ACTION_BY_ID = new Map<ClinicalJourneyActionId, ClinicalJourneyActionDefinition>(
  CORE_PATIENT_JOURNEY_ACTIONS.map((action) => [action.id, action]),
);
...
function translate(
  ...
  return interpolate(fallback ?? (typeof translated === "string" ? translated : key), values);
}
...
function journeyActionTextFallback(
  actionId: ClinicalJourneyActionId,
  field: JourneyActionTextField,
) {
  return JOURNEY_ACTION_BY_ID.get(actionId)?.[field] ?? actionId;
}

function defaultClinicalEventLabel(eventName: ClinicalEventName) {
...
function joinWithOr(t: PatientJourneyTranslator, values: readonly string[]): string {
  ...
  return values
    ...
    .reduce(
      (left, right) =>
        translate(t, "patientJourney.lists.orJoin", { left, right }, `${left} or ${right}`),
      values[0] ?? "",
    );
}
...
export function clinicalEventLabel(
  ...
  return translate(
    t,
    CLINICAL_EVENT_I18N_KEYS[eventName],
    undefined,
    defaultClinicalEventLabel(eventName),
  );
}
...
export function journeyActionLabel(
  ...
  return translate(
    t,
    journeyActionTextKey(actionId, "label"),
    undefined,
    journeyActionTextFallback(actionId, "label"),
  );
}
...
export function journeyActionShortLabel(
  ...
  return translate(
    t,
    journeyActionTextKey(actionId, "shortLabel"),
    undefined,
    journeyActionTextFallback(actionId, "shortLabel"),
  );
}
...
export function journeyActionDescription(
  ...
  return translate(
    t,
    journeyActionTextKey(actionId, "description"),
    undefined,
    journeyActionTextFallback(actionId, "description"),
  );
}
...
export function journeyBlockedReasonLabel(
  ...
  return reason
    ? translate(
        t,
        `patientJourney.blockedReasons.${reason}`,
        undefined,
...
export function journeyMessage(
  ...
  if (key === AVAILABLE_AFTER_EVENTS_KEY) {
    const events = clinicalEventList(t, activationEvents);
    return translate(t, key, { events }, `Available after ${events}`);
  }

  return translate(t, key, values ?? undefined, fallbackText ?? undefined);
}
...
export function journeyActionAvailability(
  ...
  if (action.activatesAfter.length === 0) {
    return translate(t, "patientJourney.status.available", undefined, "Available");
  }

  const events = clinicalEventList(t, action.activatesAfter);
  return translate(t, "patientJourney.status.afterEvents", { events }, `After ${events}`);
}
...
export function patientFlowItemLabel(
  ...
  return translate(t, item.labelKey, undefined, item.label);
}
...
export function patientFlowItemDescription(
  ...
  return translate(t, item.descriptionKey, undefined, item.description);
}

 ...
 */
function parseTcmsAnnotation(description) {
...
export default class KiwiTcmsReporter {
    ...
    async onBegin(config, suite) {
        ...
        const testCasesDir = this.options.testCasesDir ?? join(config.rootDir, "..", "qa", "test-cases");
        ...
        if (!product) {
            console.warn(`[kiwi-reporter] Product "${kiwiConfig.productName}" not found in Kiwi. Reporting disabled.`);
            this.client = null;
            return;
        }
        const version = await this.client.findVersion(product.id, kiwiConfig.productVersion);
        if (!version) {
            console.warn(`[kiwi-reporter] Version "${kiwiConfig.productVersion}" not found for product "${kiwiConfig.productName}". Reporting disabled.`);
            this.client = null;
            return;
        }
        // Count how many tests carry a tcms annotation so the run summary is informative
        const allTests = suite.allTests();
        const tcmsCount = allTests.filter((t) => t.annotations.some((a) => a.type === "tcms")).length;
        ...
        if (!plan) {
            ...
            console.warn(`[kiwi-reporter] Test plan "${firstPlanName}" not found. Attempting fallback.`);
            this.client = null;
            return;
        }
        this.planCache.set(firstPlanName, plan.id);
        try {
            const run = await this.client.createTestRun(plan.id, version.id, runSummary, 1);
            this.runId = run.id;
            this.active = true;
    ...
    async onTestEnd(test, result) {
        ...
        if (!parsed) {
            console.warn(`[kiwi-reporter] Invalid tcms annotation format: "${tcmsAnnotation.description}". Expected "module::summary".`);
            return;
        }
        const key = caseKey(parsed.module, parsed.summary);
        const yamlCase = this.caseMap.get(key);
        if (!yamlCase) {
            console.warn(`[kiwi-reporter] No YAML case found for key "${key}". Skipping.`);
            return;
        }
        try {
            ...
            if (caseId === null) {
                console.warn(`[kiwi-reporter] Could not find Kiwi case for "${yamlCase.summary}" in plan "${yamlCase.plan}". Skipping.`);
                return;
            }
            ...
            const message = err instanceof Error ? err.message : String(err);
            console.warn(`[kiwi-reporter] Failed to report result for "${parsed.summary}": ${message}`);
        }
    }
    async onEnd(_result) {
    ...
    async resolveCaseId(yamlCase) {
        ...
        if (planId === undefined) {
            const product = await this.client.findProduct(configFromEnv(this.options.kiwiConfig).productName);
            if (!product) {
                return null;
            }

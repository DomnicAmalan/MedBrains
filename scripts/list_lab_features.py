...
KEYWORDS = re.compile(r"lab|laboratory|lis\b|diagnostics|pathology|test.?result|specimen|sample|histopath|cytology|micro.?bio", re.IGNORECASE)
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=False), start=2):
        ...
        if header_text:
            if re.match(r"^\d+\.\s+", header_text):
                ...
                if in_lab_section or KEYWORDS.search(header_text):
                    if KEYWORDS.search(header_text):
                        in_lab_section = True
            continue
        if not feature_val: continue
        is_match = in_lab_section
        ...
        if is_match:
            ...
            total += 1
print("-" * 185)
print(f"Total lab/diagnostics features: {total}")
wb.close()
